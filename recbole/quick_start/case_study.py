...
def run_recbole_case_study(
    ...
    import numpy as np
    import torch
    #test_data.dataset['user_id']
    case_full_scores = case_study.full_sort_scores(test_data.dataset['user_id'], model, test_data, device=config["device"])
    case_topk_ = case_study.full_sort_topk(
        test_data.dataset['user_id'], model, test_data, k=5, device=config["device"]
    )
