...
class CFIT4SRec(SequentialRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        self.LPA = self.createLPAilter((self.max_seq_length, self.hidden_size), self.low_r)     #低频
        self.HPA = self.createHPAilter((self.max_seq_length, self.hidden_size), self.high_r)    #高频

        self.BSA = [self.createBSAilter((self.max_seq_length, self.hidden_size), i, 2)
                    for i in range(min(self.max_seq_length, self.hidden_size) // 2 + 1)]    #  带阻
